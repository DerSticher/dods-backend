 ...
 */
@Service
public class VoraussetzungService {

    private static final Pattern PATTERN = Pattern.compile(" ?([\\wäöüÄÖÜß \\(\\)]+)", Pattern.MULTILINE);

    private final AttributeService attributeService;

    private final ParseVoraussetzungService parseVoraussetzungService;

    private final DocumentService documentService;

    private final AbhangigkeitService abhangigkeitService;

    @Autowired
    public VoraussetzungService(AttributeService attributeService,
                                ParseVoraussetzungService parseVoraussetzungService,
                                DocumentService documentService,
                                AbhangigkeitService abhangigkeitService) {
        this.attributeService = attributeService;
        this.parseVoraussetzungService = parseVoraussetzungService;
        this.documentService = documentService;
        this.abhangigkeitService = abhangigkeitService;
    }

    public void parseVoraussetzung(Attribut attribut) {
    ...
    private boolean hasRule(Attribut attribut) {
        try {
            abhangigkeitService.findByEffektAttribut(attribut);
            return true;
